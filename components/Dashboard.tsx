...
import {
  ...
  Clock, Database, DollarSign, Info, MapPin, RefreshCw
} from "lucide-react";
...
function useInterval(cb:()=>void,ms:number|null){
  ...
  useEffect(()=>{ if(ms===null) return; const id=setInterval(()=>ref.current(),ms); return ()=>clearInterval(id)},[ms]);
}
...
function buildNextActions(violations:EchoViolation[], currentData:any, currentType:DataType|null):NextAction[]{
  ...
  if(significant.length){
    out.push({
      id:"vio-sig", title:`Resolve ${significant.length} significant violations`, category:"Compliance", priority:"Critical",
      dueDate:inDays(7),
  ...
  if(currentType==="scada" && p && p<45){
    out.push({
      id:"scada-low-pressure", title:"Investigate low pressure trend", category:"SCADA", priority:"High",
      dueDate:inDays(1),
  ...
  out.push({
    id:"fund-1", title:"Prepare SRF set-aside application draft", category:"Funding", priority:"Medium",
    dueDate:inDays(21),
...
export function Dashboard(){
  ...
  const [apiKey,setApiKey]=useLocalStorage("echo_api_key","");
  ...
  const loadSystems=async()=>{
    ...
    try{
      ...
      const raw = e?.message || "Failed to load systems";
      if (raw.includes("HTTP 403")) setSysErr("ECHO denied the request (403). Tip: set DATA_GOV_API_KEY on Vercel or use the proxy.");
      else if (raw.includes("HTTP 429")) setSysErr("Rate limited by api.data.gov (429). Try again in ~60 seconds.");
      else setSysErr(raw);
    } finally{
      setSysLoading(false);
    }
  ...
  const loadViolations=async()=>{
    ...
    try{
      ...
      const raw = e?.message || "Failed to load violations";
      if (raw.includes("HTTP 403")) setVioErr("ECHO denied the request (403). Tip: set DATA_GOV_API_KEY on Vercel or use the proxy.");
      else if (raw.includes("HTTP 429")) setVioErr("Rate limited by api.data.gov (429). Try again in ~60 seconds.");
      else setVioErr(raw);
    } finally{
      setVioLoading(false);
    }
  ...
  useEffect(()=>{ loadSystems(); loadViolations() },[]);
  useInterval(()=>{ if(auto){ loadSystems(); loadViolations() } }, auto?60000:null);
  ...
  return (
    <div className="tw120 min-h-screen bg-[var(--bg)]">
      ...
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        ...
        {/* Flash banner (NEW) */}
        {flash && (
          <div className="p-3 border rounded-lg" style={{background:"#EAF5FF", color:"#0F5E9C"}}>
            {flash}
          </div>
        )}
        ...
        <div className="card p-4">
          <div className="mb-2 row">
            <Info className="w-4 h-4 text-[var(--primary)]"/><h3 className="font-semibold">Data Source</h3>
          </div>
          <p className="text-sm text-[var(--text-muted)] mb-3">
            This pulls directly from EPA ECHO (SDWIS). Add an API key from{" "}
            <a href="https://api.data.gov/signup/" target="_blank" rel="noreferrer" className="underline">api.data.gov</a> for higher limits (optional).
          </p>
          ...
          <div className="mt-3 row">
            <button
              ...
              disabled={sysLoading || vioLoading}
              aria-busy={sysLoading || vioLoading}
              title={sysLoading || vioLoading ? "Loading…" : "Apply & Reload"}
            >
              <RefreshCw className="w-4 h-4"/> {sysLoading || vioLoading ? "Loading…" : "Apply & Reload"}
            </button>
            {(sysLoading || vioLoading) && (
              <span className="text-sm text-[var(--text-muted)]">Talking to ECHO…</span>
            )}
          </div>

          {/* Error helper messages (NEW) */}
          {(sysErr || vioErr) && (
            <div className="mt-3 p-3 border rounded-lg" style={{background:"#FFE5E5", color:"#8A1F1F"}}>
              <div className="font-semibold">We hit a problem:</div>
              <div className="text-sm">{sysErr || vioErr}</div>
              <div className="text-xs mt-1">
                Tip: Keep <b>Use Server Proxy</b> ON and set <b>DATA_GOV_API_KEY</b> in Vercel Project → Settings → Environment Variables.
              </div>
            </div>
          )}
        </div>

        {/* NOTE: Your systems/violations UI would render below this card in your full file.
                 If you want, I can wire the “no systems found” empty-state into that section as well. */}
      </div>
    </div>
  );
}
