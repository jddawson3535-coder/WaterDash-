...
type AdvisorInput = {
  ...
  pressurePsi?: number;
  flowGpm?: number;
  chlorineMgL?: number; // free chlorine
  turbidityNTU?: number;
    ...
    <main style={page}>
      <div style={container}>
        <header style={headerRow}>
          ...
          <a href="/admin/diagnostics" style={link}>
            Diagnostics
          </a>
        </header>
        ...
        <div style={card}>
          ...
          {(pwsid || stateCode) && (
            <div style={banner}>
              Running query: {`STATE=${stateCode.toUpperCase()}${pwsid ? " • PWSID=" + pwsid : ""}`}
            </div>
          )}
        </div>
        ...
        {error && (
          <div style={errorBox}>
            ...
            <div style={mutedSmall}>
              Tip: Ensure <code>DATA_GOV_API_KEY</code> is set in Vercel and the PWSID is valid.
            </div>
          </div>
        )}
          ...
          <section style={grid2}>
            ...
            <div style={card}>
              ...
              <div style={{ marginTop: 10 }}>
                <button onClick={speak} style={button(false)}>
...
const card: React.CSSProperties = {
  ...
  borderRadius: 16,
  padding: 16,
  boxShadow: "0 2px 6px rgba(15,94,156,.06),0 1px 2px rgba(14,32,51,.06)",
};
...
const button = (disabled: boolean): React.CSSProperties => ({
  background: disabled ? "#2389DA99" : "#0F5E9C",
...
function makeRecommendations(inp: AdvisorInput): string[] {
  ...
  // Starter thresholds — tune for your ops norms
  if (!Number.isNaN(lvl) && lvl < 30)
    out.push("Tank level is low (<30%). Start/verify well/booster operations; check VFD setpoints.");
  if (!Number.isNaN(psi) && psi < 35)
    out.push("Distribution pressure <35 psi. Investigate booster operation and potential main break.");
  if (!Number.isNaN(flow) && flow > 500)
    out.push("High flow >500 gpm. Confirm demand spike or suspected leak; review district meters.");
  if (!Number.isNaN(cl) && cl < 0.2)
    out.push("Free chlorine <0.2 mg/L. Increase residual (adjust dose or flush dead-ends) and resample.");
  if (!Number.isNaN(turb) && turb > 1) out.push("Turbidity >1 NTU. Check filters/clarification; investigate source water.");
  if (!Number.isNaN(calls) && calls >= 5) out.push("Multiple customer calls. Activate incident comms template and dispatch field check.");
