...
export async function GET(request: Request) {
  ...
  if (!pwsid) return NextResponse.json({ score: null, updated: null }, { status: 200 });
  if (!src)   return NextResponse.json({ score: null, updated: null }, { status: 200 });

  try {
    const r = await fetch(src, { cache: "no-store" });
    if (!r.ok) return NextResponse.json({ score: null, updated: null }, { status: 200 });
    ...
    if (ctype.includes("application/json") || src.endsWith(".json")) {
      ...
      const row = Array.isArray(arr)
        ? arr.find((x: any) => String(x.PWSID || x.pwsid).toUpperCase() === pwsid.toUpperCase())
        : null;
    ...
    // CSV source (simple parser; expects header PWSID,SCORE,UPDATED)
